          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="(good, index) in goodsList" :key="good.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <!-- 路由动态跳转的时候一定记得带参数过去，这里是params参数 -->
...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData();
      // 路径也应该动态变化:但是应该判断路径中有没有params参数，如果有params参数的话不应该清空，应该保留。
    ...
    removeprops(index) {
      // splice把索引号index的props切掉
      this.searchParams.props.splice(index, 1)
      // 重新发送数据请求
      this.getData()
    },
    // 接受子组件的自定义事件：点击品牌以后修改searchParams参数并重新请求数据
    tradmarkInfo(trademark) {
      this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`
      this.getData()
    },
    // prop商品属性自定义事件：点击品牌的商品属性以后获取对应的参数，传递给服务器，请求数据。
    attrInfo(attr, attrvalue) {
      let props = `${attr.attrId}:${attrvalue}:${attr.attrName}`
      // 判断当前props参数当中有没有这个参数，没有的话再push到searchParams当中。
    ...
    changeorder(flag) {
      ...
      let newOrder = '';
      // 判断：条件1：如果原始的flag等于当前点击的flag,表示点击了当前显示的选项
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      const o = Object.assign(this.searchParams, this.$route.query, this.$route.params);
      // 再次发起数据请求
      this.getData()
      // 每一次请求完毕，应该把相应的1、2、3级分类的id清空，让它接口下一次相应的123id
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
