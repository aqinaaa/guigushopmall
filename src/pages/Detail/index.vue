        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr, index) in spuSaleAttrList" :key="spuSaleAttrList.id">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="addOrUpdatedShopCar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      //父组件传递数据给子组件在服务器返回的数据和和空数组之间，至少返回一个空数组
  ...
  methods: {
    ...
    async addOrUpdatedShopCar() {
      // async返回的一定是一个promise函数
      // 1:发请求---将产品加入到数据库（通知服务器）
      try {
        // 成功
        await this.$store.dispatch('addOrUpdatedShopCar', { skuId: this.$route.params.skuid, skuNum: this.skuNum });
        // 在路由跳转的时候还需要将产品的信息带给下一级的路由组件
        // 本地存储|会话存储一般存储的是字符串:复杂数据通过会话存储传递过去
