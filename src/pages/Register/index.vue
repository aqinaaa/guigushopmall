<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>验证码:</label>
        <input placeholder="请输入你的验证码" v-model="code" name="code" v-validate="{ required: true, regex: /^.{4}$/ }"
...
export default {
  ...
  methods: {
    //1-- 获取验证码事件
    async getCode() {
      // ---定义了phone指向this,使用的时候就不用再单独使用this指向了
      const { phone } = this;
      // 
      try {
        await phone && this.$store.dispatch('getCode', phone)
        // 如果获取到验证码：成功，则让页面中的code=仓库中的code,让组件当中的验证码自己填写
